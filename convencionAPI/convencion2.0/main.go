...
func CreatePersonEndpoint(w http.ResponseWriter, req *http.Request) {
	...
	if person == (Person{}) {
		fmt.Fprintf(w, "Ingrese los datos nuevamente")
	} else {
...
func InscritosEndpoint(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("content-type", "application/json")
	var cont int
	collection := client.Database("Convencion").Collection("people")
	ctx, _ := context.WithTimeout(context.Background(), 30*time.Second)
...
func PromedioEndpoint(w http.ResponseWriter, req *http.Request) {
	w.Header().Set("content-type", "application/json")
	var promedio int
	var suma int
	var cont int
	...
	promedio = suma / cont
	json.NewEncoder(w).Encode(promedio)
}
...
func main() {
	...
	router.HandleFunc("/inscritos", InscritosEndpoint).Methods("GET")
	router.HandleFunc("/edadPromedio", PromedioEndpoint).Methods("GET")
